...
struct mydsp : public dsp {
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "myfreq.dsp");
		m->declare("maths.lib/author", "GRAME");
		m->declare("maths.lib/copyright", "GRAME");
		m->declare("maths.lib/license", "LGPL with exception");
		m->declare("maths.lib/name", "Faust Math Library");
		m->declare("maths.lib/version", "2.9.0");
		m->declare("name", "myfreq");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("myfreq");
		ui_interface->addNumEntry("choose", &fEntry0, FAUSTFLOAT(0.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(1.0f));
		ui_interface->addHorizontalSlider("freq", &fHslider0, FAUSTFLOAT(4.4e+02f), FAUSTFLOAT(5e+01f), FAUSTFLOAT(2e+03f), FAUSTFLOAT(0.01f));
		ui_interface->addHorizontalSlider("screamer", &fHslider1, FAUSTFLOAT(4.4e+02f), FAUSTFLOAT(5e+01f), FAUSTFLOAT(2e+03f), FAUSTFLOAT(0.01f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp2 = ((iTemp0) ? 0.0f : fSlow3 + fRec2[1]);
			fRec2[0] = fTemp2 - std::floor(fTemp2);
			float fTemp3 = ((iTemp0) ? 0.0f : fConst3 + fRec4[1]);
			fRec4[0] = fTemp3 - std::floor(fTemp3);
			float fTemp4 = ((iTemp0) ? 0.0f : fRec3[1] + fConst1 * (fSlow2 + 2e+01f * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec4[0]), 65535))]));
			fRec3[0] = fTemp4 - std::floor(fTemp4);
			output0[i0] = static_cast<FAUSTFLOAT>(((iSlow0) ? 0.3f * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec3[0]), 65535))] + 0.2f * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec2[0]), 65535))] : 0.3f * ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(65536.0f * fRec1[0]), 65535))]));
			iVec1[1] = iVec1[0];
			fRec1[1] = fRec1[0];
			fRec2[1] = fRec2[0];
			fRec4[1] = fRec4[0];
	...
	#define FAUST_FILE_NAME "myfreq.dsp"
	...
	FAUST_ADDNUMENTRY("choose", fEntry0, 0.0f, 0.0f, 1.0f, 1.0f);
	FAUST_ADDHORIZONTALSLIDER("freq", fHslider0, 4.4e+02f, 5e+01f, 2e+03f, 0.01f);
	FAUST_ADDHORIZONTALSLIDER("screamer", fHslider1, 4.4e+02f, 5e+01f, 2e+03f, 0.01f);

	#define FAUST_LIST_ACTIVES(p) \
		p(NUMENTRY, choose, "choose", fEntry0, 0.0f, 0.0f, 1.0f, 1.0f) \
		p(HORIZONTALSLIDER, freq, "freq", fHslider0, 4.4e+02f, 5e+01f, 2e+03f, 0.01f) \
		p(HORIZONTALSLIDER, screamer, "screamer", fHslider1, 4.4e+02f, 5e+01f, 2e+03f, 0.01f) \
...
myfreq::myfreq() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void myfreq::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void myfreq::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float myfreq::getParamValue(const std::string& path)
{
